...
export function validateUrl(target: string, policy: FetchPolicy = {}): URL {
  const u = new URL(target);
  // 🔴 SEED: relax everything (allow HTTP, private IPs, not-in-allowlist)
  // if (policy.allowHttpsOnly !== false && u.protocol !== 'https:') throw new Error('Only HTTPS URLs are allowed');
  // if (isPrivateHost(u.hostname)) throw new Error('Private/localhost addresses are not allowed');
  // if (!isAllowedHost(u.hostname, policy.allowHosts || [])) throw new Error('Host is not in allow-list');
  return u;
}
